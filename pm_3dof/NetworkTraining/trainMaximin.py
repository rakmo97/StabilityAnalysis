...
def max_step(x, y):
    ...
    with tf.GradientTape() as tape:
        ...
        # Compute the lagrangian value for whole training set.
        L_S = MeanLagrangian(x, y, y_pred)

    # Calculate gradients wrt multiplier
    grads = tape.gradient(-L_S, multiplier)

    print("x: {}".format(x))
    print("y: {}".format(y))
    print("y_pred: {}".format(L_S))
    print("L_S: {}".format(L_S))
    print("grads: {}".format(grads))
    print("multiplier: {}".format(multiplier))


    #  Apply gradients
    opt.apply_gradients(zip(grads, multiplier))
...
# Lagrange multiplier
multiplier = tf.Variable(0., dtype=tf.float64)
...
for epoch in range(epochs):
    ...
    for x_full_train, y_full_train in train_dataset_full_batch:
        ...
        # Max step funciton
        loss_value = max_step(x_full_train, y_full_train)
