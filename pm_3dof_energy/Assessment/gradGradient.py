...
#%% ============================================================================
# Test eigenvalue gradient calc on origin (single points)
# ============================================================================
...
with tf.GradientTape() as tape_params: # to calculate d(dVdot/dx)/dtheta
    ...
    normGrad = tf.math.reduce_euclidean_norm(grad_Vdot)


grads = tape_params.jacobian(normGrad, V_phi.trainable_weights)

print('x: {}'.format(x))
print('Vdot: {}'.format(Vdot))
print('grad_Vdot: {}'.format(grad_Vdot))
print('normGrad: {}'.format(normGrad))


#%% ============================================================================
# Test eigenvalue gradient calc on batch
# ============================================================================
...
with tf.GradientTape() as tape_params: # to calculate d(dVdot/dx)/dtheta
    ...
    grad_Vdot = t2.gradient(Vdot, x)
    normGradBatch = tf.math.reduce_euclidean_norm(grad_Vdot, axis=1)
    mean_batch_grad = tf.reduce_mean(normGradBatch)

grads = tape_params.jacobian(mean_batch_grad, V_phi.trainable_weights)

print('x: {}'.format(x))
print('Vdot: {}'.format(Vdot))
print('grad_Vdot: {}'.format(grad_Vdot))
print('normGradBatch: {}'.format(normGradBatch))
print('mean_batch_grad: {}'.format(mean_batch_grad))
